...
class MIXActorRolloutRefWorker(Worker):
    def __init__(self, config: DictConfig, role: str):
        ...
        self.role = role
        assert self.role in ['actor', 'rollout', 'ref', 'actor_rollout', 'actor_rollout_ref', "teacher_ref"]

        self._is_actor = self.role in ['actor', 'actor_rollout', 'actor_rollout_ref']
        self._is_rollout = self.role in ['rollout', 'actor_rollout', 'actor_rollout_ref']
        self._is_ref = self.role in ['ref', 'actor_rollout_ref']
        self._is_teacher_ref = self.role in ['teacher_ref']
        ...
        elif self._is_teacher_ref:
            self._is_offload_param = self.config.teacher_ref.fsdp_config.get('param_offload', False)
        ...
        if self._is_teacher_ref:
            self.config.teacher_ref.log_prob_micro_batch_size //= (self.device_mesh.shape[0] //
                                                           self.ulysses_sequence_parallel_size)
            self.config.teacher_ref.log_prob_micro_batch_size *= self.config.rollout.n
    ...
    def _build_model_optimizer(self,
        ...
        assert role in ['actor', 'ref', 'teacher_ref']
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # ##NOTE:lxy: make teacher the same embed size with actor
            # target_vocab_size = 152064 
            # if actor_module.get_input_embeddings().weight.shape[0] < target_vocab_size:
            #     print(f"[Fix] Resizing Teacher embeddings from {actor_module.get_input_embeddings().weight.shape[0]} to {target_vocab_size}")
            #     actor_module.resize_token_embeddings(target_vocab_size, mean_resizing=False)
    ...
    def init_model(self):
        ...
        if self._is_teacher_ref:
            if self.rank == 0:
                print("teacher reference model:", self.config.teacher_ref.model_path)
            self.teacher_ref_module_fsdp = self._build_model_optimizer(model_path=self.config.teacher_ref.model_path,
                                                               fsdp_config=self.config.teacher_ref.fsdp_config,
                                                               optim_config=None,
                                                               override_model_config=override_model_config,
                                                               use_remove_padding=use_remove_padding,
                                                               trust_remote_code=self.config.model.get(
                                                                   'trust_remote_code', False),
                                                               role='teacher_ref')[0]
            OmegaConf.set_struct(self.config.teacher_ref, True)
            with open_dict(self.config.teacher_ref):
                self.config.teacher_ref.use_remove_padding = use_remove_padding
            from .mix_actor import MIXDataParallelPPOActor
            self.teacher_ref_policy = MIXDataParallelPPOActor(config=self.config.teacher_ref, actor_module=self.teacher_ref_module_fsdp)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_teacher_ref_log_prob(self, data: DataProto):
        assert self._is_teacher_ref
        data = data.to('cuda')
        micro_batch_size = self.config.teacher_ref.log_prob_micro_batch_size
        data.meta_info['micro_batch_size'] = micro_batch_size
        data.meta_info['temperature'] = self.config.rollout.temperature
        data.meta_info['max_token_len'] = self.config.teacher_ref.log_prob_max_token_len_per_gpu
        ...
        # https://pytorch.org/docs/stable/notes/fsdp.html#fsdp-notes
        # unshard the root FSDP module
        self.teacher_ref_policy.actor_module._handle.reshard(True)
