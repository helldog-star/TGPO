...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, grpo_use_std=True):
    ...
    if adv_estimator == 'gae':
        ...
        from .mix_core_alg import compute_rkl_advantage
        advantages, returns = compute_rkl_advantage(
        ...
        old_log_probs = data.batch["old_log_probs"]
        teacher_log_prob = data.batch["teacher_log_prob"]
        from .mix_core_alg import compute_grpo_merge_rkl_advantage
        advantages, returns = compute_grpo_merge_rkl_advantage(
...
class MIXRayPPOTrainer(RayPPOTrainer):
    ...
    def __init__(self,
        ...
        self.use_teacher_reference_policy = Role.TeacherRefPolicy in role_worker_mapping
    ...
    def init_workers(self):
        ...
        # create teacher reference policy if needed
        if self.use_teacher_reference_policy:
            resource_pool = self.resource_pool_manager.get_resource_pool(Role.TeacherRefPolicy)
            ref_policy_cls = RayClassWithInitArgs(
                self.role_worker_mapping[Role.TeacherRefPolicy],
                config=self.config.actor_rollout_ref,
                role="teacher_ref",
            )
            self.resource_pool_to_cls[resource_pool]["teacher_ref"] = ref_policy_cls
        ...
        if self.use_teacher_reference_policy:
            self.teacher_ref_policy_wg = all_wg["teacher_ref"]
            self.teacher_ref_policy_wg.init_model()
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.use_teacher_reference_policy:
                        with _timer("teacher_log_prob", timing_raw):
                            ...
                            # teacher权重
                            batch.meta_info['teacher_coef'] = self.config.actor_rollout_ref.teacher_ref.teacher_coef
                            if self.config.actor_rollout_ref.teacher_ref.decay_rate != 0:
                                teacher_coef = batch.meta_info['teacher_coef']
                                decay_rate = self.config.actor_rollout_ref.teacher_ref.decay_rate
