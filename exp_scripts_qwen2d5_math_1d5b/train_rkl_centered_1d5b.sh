...
# ============================================================================
# OPD² Eq.(6) centered OPD advantage, cross-vocab (aligned Qwen2.5-Math-1.5B
# student vs Qwen3-30B-A3B teacher).
#
# Baseline 是 train_rkl_1d5b.sh: A_t = R_t^{OPD} = log π*(y_t) - log π_θ(y_t)
# 本实验只改 advantage:
#   A_t = R_t^{OPD}(y_t) - Σ_{v ∈ TopK(π_θ)} π_θ(v) R_t^{OPD}(v)
# k 由环境变量 K 控制 (论文实现 k=1024; OOM 时可降到 256/100):
#   K=1024 bash exp_scripts_qwen2d5_math_1d5b/train_rkl_centered_1d5b.sh
#   K=256  bash exp_scripts_qwen2d5_math_1d5b/train_rkl_centered_1d5b.sh
# ============================================================================
K=${K:-1024}
...
export EXP_NAME=rkl_centered_k${K}_qwen2d5_math_1d5b_a3b35k
...
python3 -m verl.mix_src.main_mix_ppo \
    algorithm.adv_estimator=rkl_centered \
    algorithm.topk_k=$K \
